...
export default function Afisha() {
  ...
  const movieState = useAppSelector<MovieAfishaState>(
    (x) => x.movie_afisha_reducer,
  );
  ...
  const onScroll = useCallback(
    (event) => {
      if (
        ...
        if (
          (movieState?.loadingStatusUpload === LOADING_STATUSES.IDLE ||
            movieState?.loadingStatusUpload === LOADING_STATUSES.PENDING) &&
          movieState?.isFull === false
        ) {
          setFetching(true);
        }
      }
...
export const getServerSideProps: GetServerSideProps =
  wrapper.getServerSideProps((store) => async () => {
    await store.dispatch(
      moviesAfishaGetAllAsync({ page: pageStart, limit: pageItemLimit }),
    );
    return { props: {} };
  });
