...
import { blockSeat, receiveBlockSeat, unlockSeat } from "@/modules/booking/reducer";
import { EVENTS } from "@/modules/booking/constants/events";
import { Payload, PayloadArray } from "@/modules/booking/types/payload";
import { timer_key, timerSeconds, getTimerSettings } from "@/modules/booking/container/timer-settings";
...
export default function Booking({ sessionId, userSessionId }) {
  ...
  const handleBlock = useCallback(async (sessionSeatId: string) => {
    await dispatch(blockSeat(sessionSeatId));
  }, [dispatch]);

  const handleBlockReceive = useCallback(async (sessionSeatId: string) => {
    await dispatch(receiveBlockSeat(sessionSeatId));
  }, [dispatch]);

  const handleUnlockReceive = useCallback(async (sessionSeatId: string) => {
    await dispatch(unlockSeat(sessionSeatId));
  }, [dispatch]);

  const handleUnlockAllReceive = useCallback(async (sessionSeatIds: string[]) => {
    for (const sessionSeatId of sessionSeatIds) {
      await dispatch(unlockSeat(sessionSeatId));
    }
  }, [dispatch]);

  const handleSelectSeat = useCallback(async (sessionSeatId: string) => {
  ...
  const handleCancelSelectSeat = useCallback(async (sessionSeatId: string) => {
    ...
    setSelectedSeatIds(prevState => prevState.filter(value => value !== sessionSeatId));
  ...
  const blockReceiveHandler = useCallback(async (payload: Payload) => {
  ...
  const unlockReceiveHandler = useCallback(async (payload: Payload) => {
  ...
  const unlockAllReceiveHandler = useCallback(async (payload: PayloadArray) => {
  ...
  }, [blockReceiveHandler, socket, unlockAllReceiveHandler, unlockReceiveHandler]);
      ...
      <main>
        {
          ...
          bookingState?.loadingStatus === LOADING_STATUSES.LOADING ?
            <Loading /> :
            <SessionDetailsComponent bookingState={bookingState}
