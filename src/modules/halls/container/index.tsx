...
export default function HallsContainer({ values, setFieldValue, handleBlur, errors, touched }: HallContainerProps) {
  ...
  const fetchHalls = useCallback(async (cinemaId) => {
  ...
  useEffect(() => {
    ...
    } else if (values?.hallId !== "" && values?.cinemaId !== "" && isLoading === true) {
      fetchHalls(values?.cinemaId).catch(console.error);
      setIsLoading(false);
    } else {
      setIsLoading(false);
  ...
  return (
    <HallsComponent values={values} handleChange={handleChange} hallsState={hallsState}
                    errors={errors} handleBlur={handleBlur} touched={touched}
    />
  );
}