...
import SessionCreateTimesContainer
  from "@/modules/dashboard/session-management/container/session-create-times-container";
...
export default function SessionForm({
                                      title, modalType,
                                      sessionState, initialValues,
                                      handleSubmit, handleCancel, validationSchema,
                                    }: SessionFormProps) {
  if (initialValues) {
    return (
            ...
            ({
              ...
              <Box component={Form} sx={{ mt: 3 }} onSubmit={handleSubmit}>
                ...
                >
                  ...
                  <Grid item>
                    <MoviesContainer values={values} setFieldValue={setFieldValue}
                                     errors={errors} touched={touched}
                                     handleBlur={handleBlur}
                    />
                  </Grid>
                  <Grid item>
                    <HallsContainer values={values} setFieldValue={setFieldValue}
                                    errors={errors} touched={touched}
                                    handleBlur={handleBlur}
                    />
                  </Grid>
                  <Grid item>
                    {
                      modalType === ModalActionTypes.CREATE ?
                        <SessionCreateTimesContainer values={values}
                                                     setFieldValue={setFieldValue}
                                                     errors={errors} /> :
                        modalType === ModalActionTypes.UPDATE ?
                          <SessionTimeForm values={values} errors={errors} handleChange={handleChange} /> :
                          <></>
                    }
                  </Grid>
                </Grid>
                <FormButtonGroup handleCancel={handleCancel} isLoading={
                  sessionState?.loadingStatusCreate === LOADING_STATUSES.LOADING ||
