...
export default function SessionUpdateContainer({ handleCloseModal, modalType }) {
  const dispatch = useAppDispatch();
  const sessionState = useAppSelector((x) => x.session_management_reducer);

  const [initialValues, setInitialValues] = useState<SessionUpdateFieldsType>(null);
  ...
  useEffect(() => {
    if (sessionState?.loadingStatusUpdate === LOADING_STATUSES.IDLE && wasUpdated) {
      handleCloseModal();
    }
  }, [sessionState?.loadingStatusUpdate, handleCloseModal, wasUpdated]);

  useEffect(() => {
    if (!initialValues && sessionState?.loadingStatusGetOne === LOADING_STATUSES.IDLE) {
      setInitialValues({
  ...
  }, [initialValues, sessionState?.loadingStatusGetOne, sessionState?.session?.sessionData.cinemaId, sessionState?.session?.sessionData?.coefficient, sessionState?.session?.sessionData.hallId, sessionState?.session?.sessionData.id, sessionState?.session?.sessionData.movieId, sessionState?.session?.sessionData?.startDate]);
