...
import {
  cinemaCreateAsync, cinemaDeleteAsync,
...
const initialAdapterState = cinemaManagementAdapter.getInitialState(initialState);
...
const cinemaManagementSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(cinemaGetAllActionAsync.pending.type,
        (state) => {
          state.loadingStatusGetAll = LOADING_STATUSES.LOADING;
          state.errorInfo = null;
        })
      .addCase(cinemaGetAllActionAsync.fulfilled.type,
      ...
      .addCase(cinemaGetAllActionAsync.rejected.type,
      ...
      .addCase(cinemaGetOneActionAsync.pending.type,
        (state) => {
          state.loadingStatusGetOne = LOADING_STATUSES.LOADING;
          state.errorInfo = null;
        })
      .addCase(cinemaGetOneActionAsync.fulfilled.type,
        (state, action: PayloadAction<CinemaType>) => {
          state.loadingStatusGetOne = LOADING_STATUSES.IDLE;
          state.errorInfo = null;
          state.cinema = action.payload;
        })
      .addCase(cinemaGetOneActionAsync.rejected.type,
        (state, action: PayloadAction<any>) => {
      ...
      .addCase(cinemaUpdateAsync.pending.type,
        (state) => {
          state.loadingStatusUpdate = LOADING_STATUSES.LOADING;
          state.errorInfo = null;
        })
      .addCase(cinemaUpdateAsync.fulfilled.type,
        (state, action: PayloadAction<CinemaType>) => {
          state.loadingStatusUpdate = LOADING_STATUSES.IDLE;
          state.errorInfo = null;
          cinemaManagementAdapter.updateOne(state, { id: action?.payload?.id, changes: action?.payload });
        })
      .addCase(cinemaUpdateAsync.rejected.type,
        (state, action: PayloadAction<any>) => {
      ...
      .addCase(HYDRATE, (state, action: AnyAction) => {
        if (action.payload?.cinema_management_reducer?.errorInfo) {
          state.loadingStatusGetAll = LOADING_STATUSES.FAILED;
          state.errorInfo = {
            message: action.payload?.cinema_management_reducer?.errorInfo?.message,
          ...
          state.loadingStatusGetAll = LOADING_STATUSES.IDLE;
          state.errorInfo = null;
          cinemaManagementAdapter.setAll(state, action.payload.cinema_management_reducer?.entities);
        }
      });
  },
});
...
const cinemaManagementSelectors = cinemaManagementAdapter.getSelectors<RootState>(
